...
echo DECLARE lv_schema_name varchar(100) := '%lv_schema_name%'; >> TMP\createSequence.sql
echo BEGIN >> TMP\createSequence.sql
echo 	FOR rec IN (select * from DBA_SEQUENCES WHERE sequence_owner = '%lv_schema_name%') LOOP >> TMP\createSequence.sql
...
SQLPLUS "%login_ora%/%pass_ora%@%tns_connect_to_ora%" @TMP\createSequence.sql
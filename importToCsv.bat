...
SQLPLUS "%login_ora%/%pass_ora%@%tns_connect_to_ora%" @TMP\importToCsv.sql
...
if %~6 GEQ 0 (echo SPOOL C:\CSV\SPLIT_FILE\%2_%~5.csv; >> TMP\%2.sql
for /F "tokens=1 delims=^" %%a in ("%var%") do (set nvar=%%a)
) else (echo SPOOL C:\CSV\%2.csv; >> TMP\%2.sql 
set nvar=)

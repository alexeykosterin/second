...
for /f "delims=" %%i in ('dir C:\Users\%username%\Desktop\SPOOL\EXCEPTION_BEFORE_LOAD_CSV_IN_PG\%lv_schema_name%\EXCEPTION.txt /b/a-d') do (
echo %%i >> C:\Users\%username%\Desktop\SPOOL\LOG\load_csv_%logname%.txt 2>&1
"%pg_sql_path%" -h %pg_host% -d %pg_db_name% -U %pg_user_name% -p %pg_port% -f "C:\Users\%username%\Desktop\SPOOL\EXCEPTION_BEFORE_LOAD_CSV_IN_PG\%lv_schema_name%\%%i" >> C:\Users\%username%\Desktop\SPOOL\LOG\load_csv_%logname%.txt 2>>&1
)
...
for /f "delims=" %%i in ('dir C:\Users\%username%\Desktop\SPOOL\EXCEPTION_BEFORE_LOAD_CSV_IN_PG\%lv_schema_name%\LAST_UPDATE.txt /b/a-d') do (
echo %%i >> C:\Users\%username%\Desktop\SPOOL\LOG\load_csv_%logname%.txt 2>&1
"%pg_sql_path%" -h %pg_host% -d %pg_db_name% -U %pg_user_name% -p %pg_port% -f "C:\Users\%username%\Desktop\SPOOL\EXCEPTION_BEFORE_LOAD_CSV_IN_PG\%lv_schema_name%\%%i" >> C:\Users\%username%\Desktop\SPOOL\LOG\load_csv_%logname%.txt 2>>&1
)
:end
